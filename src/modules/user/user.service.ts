...
export class UserService {
  ...
  async getBookmarks(userId: string): Promise<ResponsePayload<unknown>> {
    ...
    return ok(bookmarks, 'Daftar bookmark', { total: bookmarks.length });
  }
  ...
  async getHafalan(userId: string): Promise<ResponsePayload<unknown>> {
    const hafalan = await this.prisma.hafalan.findMany({
      where: { userId },
    ...
    return ok(hafalan, 'Daftar hafalan', { total: hafalan.length });
  }
  ...
  private nextReview(level: number): Date {
    const days = REVIEW_INTERVAL_DAYS[level] ?? 1;
